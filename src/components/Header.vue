<template>
  <header>
    <div class="container">
      <router-link to="/">
        <svg :width="120" :height="28" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 427.3 100.4">
          <path
            fill-rule="evenodd"
            clip-rule="evenodd"
            fill="#fff"
      ...
      <input type="text" class="search" v-model.trim="searchValue">
      <button @click="search" :style="{width:'60px'}">搜索</button>
      <ul class="list">
        <li>
      ...
      <div v-if="userInfo ">
        <!-- <img :src="userInfo.avatar_url" alt :style="{width:'40px'}"> -->
        <button :style="{ marginLeft: 30 }" @click="logout">登出</button>
      </div>
      <div v-else>
        <input
          type="text"
...
export default {
  ...
  methods: {
    ...
    logout() {
      // this.$store.state.login.userInfo = null;
